...
/*  Console Variables are grouped under gAdvancedResolution. (e.g. CVAR_PREFIX_ADVANCED_RESOLUTION
   ".Enabled")

    The following cvars are used in Libultraship and can be edited here:
        - Enabled                                       - Turns Advanced Resolution Mode on.
        - AspectRatioX, AspectRatioY                    - Aspect ratio controls. To toggle off, set
   either to zero.
        - VerticalPixelCount, VerticalResolutionToggle  - Resolution controls.
        - PixelPerfectMode, IntegerScale.Factor         - Pixel Perfect Mode a.k.a. integer scaling
   controls.
        - IntegerScale.FitAutomatically                 - Automatic resizing for Pixel Perfect Mode.
        - IntegerScale.NeverExceedBounds                - Prevents manual resizing from exceeding screen
   bounds.
    ...
    The following cvars are also implemented in LUS for niche use cases:
        - IgnoreAspectCorrection                        - Stretch framebuffer to fill screen.
            This is something of a power-user setting for niche setups that most people won't need or
   care about, but may be useful if playing the Switch/Wii U ports on a 4:3 television.
        - IntegerScale.ExceedBoundsBy                   - Offset the max screen bounds, usually by +1.
                                                          This isn't that useful at the moment, so it's
   unused here.
*/
...
void ResolutionCustomWidget(WidgetInfo& info) {
    ...
    UIWidgets::CVarCheckbox("Set fixed vertical resolution (disables resolution slider)",
                            ...
                            UIWidgets::CheckboxOptions({ { .disabled = disabled_everything } })
                                .Tooltip("Override the resolution scale slider and use the settings "
                                         "below, irrespective of window size.")
                                .Color(THEME_COLOR));
    ...
    if (ImGui::CollapsingHeader("Integer Scaling Settings", IntegerScalingResolvedImGuiFlag)) {
        ...
        UIWidgets::CVarCheckbox("Automatically scale image to fit viewport",
                                ...
                                UIWidgets::CheckboxOptions({ { .disabled = disabled_pixelPerfectMode } })
                                    .DefaultValue(true)
                                    .Color(THEME_COLOR)
                                    .Tooltip("Automatically sets scale factor to fit window. Only available in Pixel "
                                             "Perfect Mode."));
    ...
    if (ImGui::CollapsingHeader("Additional Settings")) {
        ...
        // This option is only really needed on systems limited to 16:9 TV resolutions, such as
        // consoles. The associated cvar is still functional on PC platforms if you want to use it
        // though.
        UIWidgets::CVarCheckbox(
        ...
        // Beginning of Integer Scaling additional settings.
        {
            ...
            if (UIWidgets::CVarCheckbox(
                    ...
                    UIWidgets::CheckboxOptions({ { .disabled = disabled_neverExceedBounds } })
                        .Tooltip("Prevents integer scaling factor from exceeding screen bounds.\n\n"
                                 "Enabled: Will clamp the scaling factor and display a gentle warning "
                                 "in the "
                                 "resolution editor.\n"
                                 "Disabled: Will allow scaling to exceed screen bounds, for users who "
                                 "want to crop "
                                 "overscan.\n\n"
                                 " " ICON_FA_INFO_CIRCLE " Please note that exceeding screen bounds "
                                 "may show a scroll bar on-screen.")
                        .Color(THEME_COLOR)
                        .DefaultValue(true))) {
            ...
            // A popular feature in some retro frontends/upscalers, sometimes called "crop overscan" or
            // "1080p 5x".
            UIWidgets::CVarCheckbox(
            ...
            // It does actually function as expected, but exceeding the bottom of the screen shows a
            // scroll bar. I've ended up commenting this one out because of the scroll bar, and for
            // simplicity.
...
void RegisterResolutionWidgets() {
    ...
    mGhostshipMenu->AddWidget(path, "Viewport dimensions: {} x {}", WIDGET_TEXT)
        .RaceDisable(false)
    ...
    mGhostshipMenu->AddWidget(path, "Internal resolution: {} x {}", WIDGET_TEXT)
        .RaceDisable(false)
        .PreFunc([](WidgetInfo& info) {
    ...
    //  Activator
    mGhostshipMenu->AddWidget(path, "Enable advanced settings.", WIDGET_CVAR_CHECKBOX)
    ...
    mGhostshipMenu->AddWidget(path, ICON_FA_QUESTION_CIRCLE " \"N64 Mode\" is overriding these settings.", WIDGET_TEXT)
        .RaceDisable(false)
        .PreFunc([](WidgetInfo& info) { info.isHidden = !CVarGetInteger(CVAR_LOW_RES_MODE, 0); })
        .Options(TextOptions().Color(Colors::LightBlue));
    mGhostshipMenu->AddWidget(path, "Click to disable N64 mode", WIDGET_BUTTON)
        .RaceDisable(false)
        .PreFunc([](WidgetInfo& info) { info.isHidden = !CVarGetInteger(CVAR_LOW_RES_MODE, 0); })
    ...
    // Aspect Ratio
    mGhostshipMenu->AddWidget(path, "AspectSep", WIDGET_SEPARATOR).RaceDisable(false).PreFunc([](WidgetInfo& info) {
        if (mGhostshipMenu->GetDisabledMap().at(DISABLE_FOR_ADVANCED_RESOLUTION_OFF).active) {
            info.activeDisables.push_back(DISABLE_FOR_ADVANCED_RESOLUTION_OFF);
        }
    });
    mGhostshipMenu->AddWidget(path, "Force aspect ratio:", WIDGET_TEXT)
        .RaceDisable(false)
        .PreFunc([](WidgetInfo& info) {
            if (mGhostshipMenu->GetDisabledMap().at(DISABLE_FOR_ADVANCED_RESOLUTION_OFF).active) {
                info.activeDisables.push_back(DISABLE_FOR_ADVANCED_RESOLUTION_OFF);
    ...
    mGhostshipMenu->AddWidget(path, "(Select \"Off\" to disable.)", WIDGET_TEXT)
        .RaceDisable(false)
        .PreFunc([](WidgetInfo& info) {
            if (mGhostshipMenu->GetDisabledMap().at(DISABLE_FOR_ADVANCED_RESOLUTION_OFF).active) {
                info.activeDisables.push_back(DISABLE_FOR_ADVANCED_RESOLUTION_OFF);
    ...
    // Presets
    mGhostshipMenu->AddWidget(path, "Aspect Ratio", WIDGET_COMBOBOX)
    ...
    mGhostshipMenu->AddWidget(path, "AspectRatioCustom", WIDGET_CUSTOM)
        ...
        .CustomFunction([](WidgetInfo& info) {
            ...
            if (item_aspectRatio == default_aspectRatio && !showHorizontalResField) {
                // Declare input interaction bools outside of IF statement to prevent Y field from
                // disappearing.
                const bool input_X =
    ...
    mGhostshipMenu->AddWidget(path, "MoreResolutionSettings", WIDGET_CUSTOM)
...
void UpdateResolutionVars() {
    ...
    // Letting it go below 1 in this Editor will even allow for checking if screen bounds are being
    // exceeded.
