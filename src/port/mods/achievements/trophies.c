...
volatile int trp_id;
SceUID trp_request_mutex, trp_free_mutex;
int trophies_unlocker(SceSize args, void *argp) {
	for (;;) {
		...
		sceKernelSignalSema(trp_free_mutex, 1);
	}
}

int trophies_init() {
	// Starting sceNpTrophy
	strcpy(comm_id, "GHST00001");
	...
	if (res < 0) {
#ifdef DEBUG
		sceClibPrintf("sceNpTrophyCreateContext returned 0x%08X\n", res);
#endif	
		return res;
	}
	...
	trp_free_mutex = sceKernelCreateSema("trps free", 0, 1, 1, NULL);
	sceClibPrintf("Trophy mutexes: %x %x\n", trp_request_mutex, trp_free_mutex);
	SceUID tropies_unlocker_thd = sceKernelCreateThread("trophies unlocker", &trophies_unlocker, 0x10000100, 0x40000, 0, 0, NULL);
	sceClibPrintf("sceKernelCreateThread(trophies_unlocker): %X\n", tropies_unlocker_thd);
	sceClibPrintf("sceKernelStartThread(trophies_unlocker): %X\n", sceKernelStartThread(tropies_unlocker_thd, 0, NULL));
...
void trophies_unlock(uint32_t id) {
	if (trophies_available && !trophies_is_unlocked(id)) {
		sceKernelWaitSema(trp_free_mutex, 1, NULL);
